...
[RapidKit Core](https://github.com/chistiq/rapidkit-core) and retain their
legacy metadata as compatibility input. Their current workspace interface and
all new commands use [Workspai](https://github.com/chistiq/workspai).
...
If the locked resolver cannot reproduce a project, the wrapper falls back to
registry replay and accepts the result only when every restored module still
matches the committed version exactly. The user-facing command stays stable
and the fallback cannot silently upgrade or downgrade an example.
...
| Repair a Doctor or governance blocker safely | [Governed repair transaction](WORKSPACE_ONBOARDING.md#run-a-governed-repair-transaction) |
...
- project-level `.rapidkit/modules.lock.yaml` files for Core module restore

The canonical command for refreshing model, graph, health, contracts,
readiness, verification, and agent grounding in contract order is:
...
| Workspai CLI | [chistiq/workspai](https://github.com/chistiq/workspai) |
| VS Code extension | [chistiq/rapidkit-vscode](https://github.com/chistiq/rapidkit-vscode) |
| RapidKit Core | [chistiq/rapidkit-core](https://github.com/chistiq/rapidkit-core) |
